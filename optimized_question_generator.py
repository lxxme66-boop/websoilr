...
class QuestionGenerator:
    ...
    def __init__(self, config: dict):
        ...
        # 合理性检查配置
        self.rationality_threshold = 0.7  # 合理性分数阈值
    ...
    def generate_questions(self, subgraphs: List[Dict]) -> List[Dict]:
        ...
        # 批量优化问题
        logger.info("开始优化问题质量...")
        optimized_qa_pairs = self._batch_optimize_questions(all_qa_pairs)
        
        # 后处理和质量检查
        filtered_qa_pairs = self._filter_qa_pairs(optimized_qa_pairs)
    ...
    def _batch_optimize_questions(self, qa_pairs: List[Dict]) -> List[Dict]:
        """批量优化问题，提高自然度和合理性"""
        optimized_pairs = []
        ...
        for i in range(0, len(qa_pairs), batch_size):
            ...
            for qa in batch:
                # 先检查合理性
                is_rational, rationality_score = self._check_question_rationality(qa)
                
                if is_rational:
                    # 如果合理但可以改进，进行优化
                    if rationality_score < 0.9:
                        optimized_qa = self._optimize_single_question(qa)
                        optimized_pairs.append(optimized_qa)
                    else:
                        optimized_pairs.append(qa)
                else:
                    # 不合理的问题，尝试修复
                    fixed_qa = self._fix_irrational_question(qa)
                    if fixed_qa:
                        optimized_pairs.append(fixed_qa)
                    # 否则丢弃
        
        logger.info(f"优化完成: {len(qa_pairs)} -> {len(optimized_pairs)} (丢弃 {len(qa_pairs) - len(optimized_pairs)} 个不合理问题)")
        
        return optimized_pairs
    
    def _check_question_rationality(self, qa: Dict) -> Tuple[bool, float]:
        """检查问题的合理性"""
        question = qa['question']
        subgraph = qa['subgraph']
        
        # 构造合理性检查提示
        prompt = f"""请评估以下问题的合理性和自然度。
...
"""
        ...
        try:
            # 提取评分
            if "评分：" in response:
                score_line = response.split("评分：")[1].split("\n")[0]
                score = float(score_line.strip())
            else:
                score = 0.5  # 默认分数
            
            # 提取是否合理
            is_rational = "合理：是" in response or score >= self.rationality_threshold
            
            return is_rational, score
        except:
            # 解析失败，保守处理
            return True, 0.7
    
    def _optimize_single_question(self, qa: Dict) -> Dict:
        """优化单个问题，使其更自然"""
        question = qa['question']
        q_type = qa['type']
        lang = qa['language']
        ...
        # 构造优化提示
        prompt = f"""请优化以下问题，使其更加自然、流畅和符合日常表达习惯。
...
3. 避免生硬的模板痕迹
4. 确保语法正确、语义清晰
5. 如果是中文，使用地道的中文表达；如果是英文，使用地道的英文表达
...
优化后的问题："""
        ...
        # 提取优化后的问题
        if "优化后的问题：" in optimized_text:
            optimized_question = optimized_text.split("优化后的问题：")[-1].strip()
        else:
            optimized_question = question  # 保持原样
        
        # 更新QA对
        optimized_qa = qa.copy()
        optimized_qa['question'] = optimized_question
        optimized_qa['optimized'] = True
        
        return optimized_qa
    
    def _fix_irrational_question(self, qa: Dict) -> Optional[Dict]:
        """修复不合理的问题"""
        question = qa['question']
        subgraph = qa['subgraph']
        q_type = qa['type']
        ...
        # 构造修复提示
        prompt = f"""以下问题存在合理性问题，请根据知识图谱信息重新生成一个合理的问题。
...
新问题："""
        ...
        # 提取新问题
        if "新问题：" in response:
            new_question = response.split("新问题：")[-1].strip()
            
            # 验证新问题
            if len(new_question) > 10 and len(new_question) < 200:
                # 创建新的QA对
                fixed_qa = qa.copy()
                fixed_qa['question'] = new_question
                fixed_qa['fixed'] = True
                
                # 重新生成答案
                fixed_qa['answer'] = self._generate_answer_with_llm(
                    new_question, subgraph, q_type
                )
    ...
    def _generate_answer_with_llm(self, question: str, subgraph: Dict, 
        ...
        """使用LLM生成答案（改进版）"""
        # 构造提示
        prompt = self._create_enhanced_answer_prompt(question, subgraph, q_type)
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                max_new_tokens=300,  # 增加长度以生成更详细的答案
        ...
        # 验证答案质量
        if self._is_answer_too_simple(answer, q_type):
            # 如果答案太简单，重新生成更详细的答案
            answer = self._regenerate_detailed_answer(question, subgraph, q_type, answer)
        
        return answer
    
    def _create_enhanced_answer_prompt(self, question: str, subgraph: Dict, 
                                     q_type: str) -> str:
        """创建增强的答案生成提示"""
        # 将子图信息格式化
        ...
        # 根据问题类型定制提示
        type_specific_instruction = self._get_type_specific_instruction(q_type)
...
请提供一个准确、详细且信息丰富的答案。避免简单重复图谱中的信息，而是要：
1. 整合相关信息
2. 进行必要的推理
3. 提供有价值的见解
4. 使用自然流畅的语言
...
答案："""
    ...
    def _get_type_specific_instruction(self, q_type: str) -> str:
        """获取特定问题类型的指导"""
        instructions = {
            'factual': "请基于事实信息给出明确的答案，并适当解释相关背景。",
            'comparison': "请从多个维度进行比较，指出主要差异和各自特点。",
            'reasoning': "请展示推理过程，说明因果关系或逻辑联系。",
            'multi_hop': "请清晰地展示信息之间的连接路径，说明中间环节。",
            'counterfactual': "请基于现有信息，合理推测替代方案或可能的结果。",
            'temporal': "请按时间顺序说明发展过程或变化趋势。",
            'causal': "请分析因果关系，说明原因和影响。"
        }
        
        return instructions.get(q_type, "请给出全面准确的答案。")
    
    def _is_answer_too_simple(self, answer: str, q_type: str) -> bool:
        """检查答案是否过于简单"""
        # 基本长度检查
        if len(answer) < 20:
            return True
        ...
        # 检查是否只是简单重复实体和关系
        if q_type == 'factual' and len(answer.split()) < 10:
            return True
        
        # 检查是否缺乏解释
        if q_type in ['reasoning', 'comparison', 'multi_hop'] and len(answer) < 50:
            return True
        
        return False
    
    def _regenerate_detailed_answer(self, question: str, subgraph: Dict, 
                                  q_type: str, simple_answer: str) -> str:
        """重新生成更详细的答案"""
        prompt = f"""原始答案过于简单，请提供更详细的答案。
...
详细答案："""
        ...
        detailed_answer = self.tokenizer.decode(outputs[0], skip_special_tokens=True)
        
        if "详细答案：" in detailed_answer:
            return detailed_answer.split("详细答案：")[-1].strip()
        else:
            return simple_answer  # 保持原答案
    ...
    def _filter_qa_pairs(self, qa_pairs: List[Dict]) -> List[Dict]:
        ...
        for qa in qa_pairs:
            ...
            if quality_config['answer_validation']:
                if not qa['answer'] or a_len < min_a_len:
                    continue
            
            # 去重（改进的去重策略）
            # 使用问题的前50个字符作为去重键
            question_key = qa['question'][:50].lower().strip()
